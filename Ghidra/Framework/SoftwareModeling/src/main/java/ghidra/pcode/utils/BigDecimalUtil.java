...
public class BigDecimalUtil {
	public static final int SCALE = 100; // Default value is 18
	...
	public static BigDecimal log(BigDecimal base, BigDecimal val) {
		return BigDecimalUtil.ln(val, SCALE)
				.divide(BigDecimalUtil.ln(base, SCALE),
						SCALE + 1, ROUNDING_MODE);
	}
	...
	public static BigDecimal getPi(int num) {
		...
		for (int i = 0; i < num; i++) {
			...
			temp4 = one.divide(((eight.multiply(ii)).add(five)),
					new MathContext(num, RoundingMode.HALF_DOWN));
