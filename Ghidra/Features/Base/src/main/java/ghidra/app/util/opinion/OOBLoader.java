...
public class OOBLoader extends AbstractProgramLoader {

	public final static String BINARY_NAME = "Raw Binary with out-of-band bytes";

	public static final String OPTION_NAME_LEN_DATA = "Length of actual data";
	public static final String OPTION_NAME_LEN_OOB = "Length of out-of-band bytes";
	...
	public String validateOptions(ByteProvider provider, LoadSpec loadSpec, List<Option> options,
		...
		for (Option option : options) {
			...
			try {
				if (optName.equals(OPTION_NAME_BASE_ADDR)) {
				...
				else if (optName.equals(OPTION_NAME_LEN_DATA)) {
					try {
					...
					if (length <= 0) {
						return OPTION_NAME_LEN_DATA + " must be overriden to positive integer";
					}
				}
				else if (optName.equals(OPTION_NAME_LEN_OOB)) {
					try {
						length = parseLong(option);
					}
					catch (Exception e) {
					...
					if (length <= 0) {
						return OPTION_NAME_LEN_OOB + " must be overriden to positive integer";
					}
				}
	...
	private long getLengthData(List<Option> options) {
		long length = 0;
		if (options != null) {
			for (Option option : options) {
				String optName = option.getName();
				if (optName.equals(OPTION_NAME_LEN_DATA)) {
					length = parseLong(option);
				}
			}
		}
	...
	private long getLengthOOB(List<Option> options) {
		long length = 0;
		if (options != null) {
			for (Option option : options) {
				String optName = option.getName();
				if (optName.equals(OPTION_NAME_LEN_OOB)) {
					length = parseLong(option);
				}
			}
		}
	...
	protected void loadProgramInto(ByteProvider provider, LoadSpec loadSpec,
		...
		long length = provider.length(); // Use actual file length provided and put all pages possible
		long fileOffset = getFileOffset(options);
		boolean isOverlay = isOverlay(options);
		long lengthData = getLengthData(options);
		long lengthOOB = getLengthOOB(options);
		if (lengthData <= 0 || lengthOOB <= 0) {
			throw new LoadException("Please open options dialog and fix errors");
		}
		...
		Address currentAddr = baseAddr;

		long lengthPage = lengthData + lengthOOB;
		long numPages = length / lengthPage;
		for (long i = 0; i < numPages; i++) {
			String currentName = blockName + i;
			try {
				FileBytes fileBytes = MemoryBlockUtils.createFileBytes(prog, provider, currentOffset, lengthData, monitor);
				createBlock(prog, isOverlay, currentName, currentAddr, fileBytes, lengthData, log);
			} catch (IOException | AddressOverflowException e) {
				String errorDescription = "Couldn't load page. ";
				errorDescription += "Further information on error: ";
				errorDescription += "current page number: " + i;
				errorDescription += ", load address: " + currentAddr;
				errorDescription += ", file offset: " + currentOffset;
				throw new LoadException(errorDescription);
			}
			currentOffset += lengthPage;
			currentAddr = currentAddr.add(lengthData);
		}
	}
	...
	public List<Option> getDefaultOptions(ByteProvider provider, LoadSpec loadSpec,
		...
		list.add(new Option(OPTION_NAME_LEN_DATA, new HexLong(0), HexLong.class,
			Loader.COMMAND_LINE_ARG_PREFIX + "-length"));
		list.add(new Option(OPTION_NAME_LEN_OOB, new HexLong(0), HexLong.class,
				Loader.COMMAND_LINE_ARG_PREFIX + "-length"));
