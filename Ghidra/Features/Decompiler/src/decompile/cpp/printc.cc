...
void PrintC::opExtractInd(const PcodeOp *op)

{
  ostringstream s;
  s << "EXTRACTIND" << op->getIn(1)->getSize() << "_" << op->getOut()->getSize();
...
void PrintC::opInsertInd(const PcodeOp *op)
...

{
  ...
  if (nm == Funcdata::FUNCTION_INSERTIND) {
    opInsertInd(op);
    return;
  }
...

{
  ...
  if (ct->isPieceStructured()) {
    ...
    // Use artificial slot
    const TypeField *field = ct->findTruncation(byteOff,sz,op,1,offset);
    // A formal structure field
    if (field != (const TypeField*)0) {
      // Check size and offset
      if (field->type->getSize() == sz && offset == 0) {
