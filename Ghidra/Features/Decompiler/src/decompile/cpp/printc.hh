...
protected:
  ...
  bool emitAlloca(const PcodeOp *op);
  ...
  void opExtractInd(const PcodeOp *op);
  void opInsertInd(const PcodeOp *op);
  ...
  virtual void emitVarDeclStatement(const Symbol *sym,const Funcdata *fd);
  ...
  virtual bool checkPrintZeroInitializer(const Symbol *sym,const Funcdata *fd);
...
public:
  ...
  virtual void emitBlockMultiLabel(const BlockMultiLabelClause *bl);
